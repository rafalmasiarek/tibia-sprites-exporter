...
#### Optional: JSON index for grouped output
If you want a machine-readable mapping for downstream tooling, you can emit a JSON index file:

```bash
./tibia-sprites-exporter group   --splitOutput ./output/split   --groupedOutput ./output/grouped   --groupsIndexOut ./output/groups-index.json
```

The JSON index includes, per detected group:
- `first_sprite_id`, `last_sprite_id`, `count`
- full `sprite_ids` list
- `output_base` and `output_png` for the grouped PNG filename

## Configuration and Defaults
This CLI uses Viper for configuration. Settings can come from, in order of precedence:
...
groupsIndexOut: ./output/groups-index.json
```
...
- Prefix: `TSE_`. Keys are uppercased and use underscores. Examples:
  ...
  - `TSE_GROUPSINDEXOUT=./output/groups-index.json`
...
- `group --groupsIndexOut <path>` – Optional JSON index output path (no file is written unless set).
...
output/
  ...
  groups-index.json  # Optional JSON index (when --groupsIndexOut is used)
```
